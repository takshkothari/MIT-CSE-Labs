...
int main(void) {
    ...
    char *res = (char*)malloc(sizeof(char)*len * n);
    ...
    cl_mem t_mem_obj = clCreateBuffer(context, CL_MEM_WRITE_ONLY,len*sizeof(char) * n, NULL, &ret);
    ...
    cl_kernel kernel = clCreateKernel(program, "strmul", &ret);
    ...
    size_t global_item_size = n*len; // Process the entire lists
    ...
    ret = clEnqueueReadBuffer(command_queue, t_mem_obj, CL_TRUE, 0,len*sizeof(char) * n, res, 0, NULL, NULL);
    // res[len * n]='\0';
    printf("\nResultant string: %s\n",res);
